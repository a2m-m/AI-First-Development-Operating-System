...
#### 3-1. ローカル除外ルールの設定（`.git/info/exclude`）

OS インフラのローカル除外ルールは **`.gitignore` ではなく `.git/info/exclude` に書く**。

- `.git/info/exclude` はローカル git にのみ存在し、GitHub には上がらない
- `.gitignore` を汚さないため、instance の `.gitignore` はプロジェクト固有の内容だけを保てる

`.git/info/exclude` を読んで以下が含まれているか確認し、**不足分を末尾に追記する**：

```
# OS Infrastructure（ローカル開発用、GitHub には上げない）
os_scripts/
os_docs/
...
```bash
# exclude ファイルが存在しない場合は作成する
mkdir -p .git/info
touch .git/info/exclude

# 不足エントリを追記
grep -qF "os_scripts/" .git/info/exclude || echo "os_scripts/" >> .git/info/exclude
grep -qF "os_docs/" .git/info/exclude || echo "os_docs/" >> .git/info/exclude
grep -qF ".claude/settings.local.json" .git/info/exclude || echo ".claude/settings.local.json" >> .git/info/exclude
```

次に、**すでに tracked なファイルを git の追跡対象から外す**：

```bash
git rm -r --cached os_scripts/ os_docs/ 2>/dev/null || true
```

`project_config.yml` と `.claude/skills/` は Instance が管理するファイルのため untrack しない。
untrack 後はそのままコミットする（`.gitignore` は変更しない）：

```bash
git add -u
git commit -m "chore: OS インフラファイルを untrack"
```

なお `.env` / `*.log` など秘匿・汎用エントリは instance の `.gitignore` に追記してよい（GitHub に上げても問題ない共通設定のため）：
...
- **自動修正してよいもの**: プレースホルダ置換、`.git/info/exclude` エントリ追記、`.gitignore` への秘匿エントリ追記（`.env` 等）、tracked OS インフラファイル（os_scripts/ os_docs/）の `git rm --cached`、.ai-context.md リセット、資材のコピー（テンプレパス判明時）、GitHub remote の切り替え
