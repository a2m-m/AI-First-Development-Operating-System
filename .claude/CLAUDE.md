...
- **Related Issues** に同じ機能グループの Issue を列挙する
- 機能グループを表す `scope/xxx` ラベルを付与する（例: `scope/auth`, `scope/design`, `scope/ci`）
...
- **作業終了後**：`.ai-context.md`（または `.ai-context.local.md`）を更新し、次のセッションが迷子にならない状態にする
- **OS Template Repo では雛形を維持**：`.ai-context.md` はセクション枠＋TODO のみの雛形を保つ。OS 開発リポジトリ固有の状態（進行中 Issue・PR・Decisions 等）は `.ai-context.local.md` に書く
- **`.ai-context.local.md`**：OS 開発リポジトリ専用の状態ファイル。`.git/info/exclude` で除外済みのため GitHub には上がらず、Template 複製時にも引き継がれない
...
| コマンド | 用途 | 引数例 | 起動者 |
|---|---|---|---|
| `/plan` | Issue AC 確認 → コード探索 → 実装計画立案 | — | ユーザー / Claude（自動） |
...
| `.ai-context.md` | テンプレート雛形（セクション枠のみ） |
| `.ai-context.local.md` | OS 開発リポジトリ固有の状態・進行中 Issue・次の手（`.git/info/exclude` で除外） |
